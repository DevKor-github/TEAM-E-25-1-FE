...
export const useAdminAuthStore = create(
  persist<AdminAuthState>(
    (set) => ({
      ...
      // 로그아웃 함수
      logout: () => {
        localStorage.removeItem("admin-auth-expiration");
        set({ isLoggedIn: false });
      },

      // 만료 확인 함수
      checkExpiration: () => {
        const expiration = localStorage.getItem("admin-auth-expiration"); // 로그아웃 상태면 expiration이 null
