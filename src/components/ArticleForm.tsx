  ...
  const articleFormSchema = z.object({
    ...
    content: z.string({
      required_error: "행사 내용을 입력하세요.",
    }).min(1, { message: "행사 내용을 입력하세요." })
    ...
    thumbnail_image: z
      ...
      .refine((files) => ALLOWED_FILE_TYPES.includes(files?.[0]?.type), "jpg 또는 png 파일만 업로드 가능합니다."),

    detail_image: z
      .custom<FileList>()
      .optional()
      .refine((files) => !files || files.length <= 10, "상세 이미지는 최대 10개까지만 업로드할 수 있습니다.")
      .refine((files) => !files || Array.from(files).every((file) => file.size <= MAX_FILE_SIZE), "각 파일의 크기는 5MB 이하여야 합니다.")
      .refine((files) => !files || Array.from(files).every((file) => ALLOWED_FILE_TYPES.includes(file.type)), "jpg 또는 png 파일만 업로드 가능합니다."),
  }).refine(
    (data) => {
  ...
  export function ArticleForm({
    ...
    defaultValues: {
      ...
      event_type: defaultValues?.event_type ?? undefined,
      location: defaultValues?.location ?? "",
      start_datetime: defaultValues?.start_datetime ?? "",
      end_datetime: defaultValues?.end_datetime ?? "",
      content: defaultValues?.content ?? "",
      link: defaultValues?.link ?? "",
      thumbnail_image: defaultValues?.thumbnail_image ?? undefined,
      detail_image: defaultValues?.detail_image ?? undefined,
    },
    mode: "onTouched",
    criteriaMode: "all",
